...
if __name__=='__main__':
    ...
    cv2.createTrackbar(low_H_name, mask_window_name , low_H, max_value_H, on_low_H_thresh_trackbar)
    cv2.createTrackbar(high_H_name, mask_window_name , high_H, max_value_H, on_high_H_thresh_trackbar)
    cv2.createTrackbar(low_S_name, mask_window_name , low_S, max_value, on_low_S_thresh_trackbar)
    cv2.createTrackbar(high_S_name, mask_window_name , high_S, max_value, on_high_S_thresh_trackbar)
    cv2.createTrackbar(low_V_name, mask_window_name , low_V, max_value, on_low_V_thresh_trackbar)
    cv2.createTrackbar(high_V_name, mask_window_name , high_V, max_value, on_high_V_thresh_trackbar)
    while True:
        #mask = filterImageHSV()
