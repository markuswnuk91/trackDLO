...
def testPointCloudExtraction():
    ...
    cameraCenter = np.linalg.inv(preProcessor.calibrationParameters["T_Camera_To_RobotBase"])[:3,3]
    robotBase = np.array([0,0,0])
    # Visualization
    if vis:
        fig = plt.figure()
        ...
        plotPoint(ax = ax, x = cameraCenter, size = 50, color = [1,0,0])
        plotPoint(ax=ax, x =robotBase, size = 50, color = [0,0,1])
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_zlabel("Z")
        plt.show(block=True)
