...
class PreProcessing(PointCloudProcessing, ImageProcessing):
    ...
    """
    def __init__(self, calibrationParameters = None, cameraParameters = None, hsvFilterParameters = None, roiFilterParameters = None, boundingBoxParameters = None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.calibrationParameters = self.loadCalibrationParameters(fileName = "calibrationParameters.json", folderPath = "config/calibration/") if calibrationParameters is None else calibrationParameters
        self.cameraParameters = self.loadCameraParameters("cameraParameters.json") if cameraParameters is None else cameraParameters
        self.hsvFilterParameters = self.getFilterDefaultValues_HSV() if hsvFilterParameters is None else hsvFilterParameters
        self.roiFilterParameters = self.getFilterDefaultValues_ROI() if roiFilterParameters is None else roiFilterParameters
        self.boundingBoxParameters = self.getBoundingBoxDefaultValues() if boundingBoxParameters is None else boundingBoxParameters

    def getFilterDefaultValues_HSV():
        hsvDefaultValues = {"hueMin":0, "hueMax": 180, "saturationMin":0, "saturationMax":255, "valueMin":0, "valueMax":255}
        return hsvDefaultValues
    
    def getFilterDefaultValues_ROI():
        roiDefaultValues = {"uMin":0, "uMax": 1, "vMin":0, "vMax":1}
        return roiDefaultValues
    
    def getBoundingBoxDefaultValues():
        boundingBoxParameters = {"xMin":-1, "xMax": 1, "yMin":-1, "yMax":1, "zMin":0, "zMax":2.5}
    ...
    def calculatePointCloudFiltered_2D(self,rgbImage: np.array, disparityMap: np.array, qmatrix: np.ndarray = None, hsvFilterParameters: dict = None, roiFilterParameters: dict = None):
        ...
        # Color Filter
        (hueMin, hueMax, saturationMin, saturationMax, valueMin, valueMax) = self.getParametersFromDict_HSV(hsvFilterParameters)
        maskFilter_Color = self.getMaskFromRGB_applyHSVFilter(rgbImage, hueMin, hueMax, saturationMin, saturationMax, valueMin, valueMax)
        # ROI Filter
        ...
        points, colors = self.calculatePointCloud(rgbImage, disparityMap, qmatrix, combinedMask)
        return points, colors
    
    def calculatePointCloudFiltered_2D_3D(self,rgbImage: np.array, disparityMap: np.array, qmatrix: np.ndarray = None, hsvFilterParameters: dict = None, roiFilterParameters: dict = None, boundingBoxParameters: dict = None):
        if boundingBoxParameters is None:
            boundingBoxParameters = self.boundingBoxParameters
        if qmatrix is None:
            qmatrix = self.cameraParameters["qmatrix"]
        # PointCloud
        points, colors = self.calculatePointCloudFiltered_2D(rgbImage, disparityMap, qmatrix, hsvFilterParameters, roiFilterParameters)
        #Bounding Box Filter
        (xMin, xMax, yMin, yMax, zMin, zMax) = self.getParametersFromDict_BoundingBox(boundingBoxParameters)
        mask_BoundingBox = self.getMaskFromBoundingBox(points, xMin, xMax, yMin, yMax, zMin, zMax)
