...
if __name__=='__main__':
    ...
    cv2.createTrackbar(left_boarder_name, mask_window_name , leftBoarder, imageWidth, on_left_boarder_thresh_trackbar)
    cv2.createTrackbar(right_boarder_name, mask_window_name , rightBoarder, imageWidth, on_right_boarder_thresh_trackbar)
    cv2.createTrackbar(lower_boarder_name, mask_window_name , lowerBoarder, imageHeight, on_lower_boarder_thresh_trackbar)
    cv2.createTrackbar(upper_boarder_name, mask_window_name , upperBoarder, imageHeight, on_upper_boarder_thresh_trackbar)
    while True:
        ...
        frame_diff[np.where((frame_diff == [0,0,0]).all(axis=2))] = [255,255,255] #invert such that filtered values are displayed white
